...
import { Send, MessageSquare, UserX, RotateCcw, Loader } from "lucide-react"
...
const ChatInterface = ({ onDisconnect, onFindNew }: ChatInterfaceProps) => {
  ...
  const handleSendMessage = () => {
    if (!inputMessage.trim() || connectionStatus !== "connected" || !socket) return
  ...
  const getStatusText = () => {
    switch (connectionStatus) {
      ...
      case "connected":
        return `You are now chatting with ${otherUser?.name || "a college mate"}!`
      case "disconnected":
      ...
      <div className="bg-white/80 backdrop-blur-sm border-b border-gray-200 p-4 shadow-sm">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-800">CampusTalk Text</h1>
              <div className="flex items-center space-x-2">
                {connectionStatus === "connecting" && <Loader className="w-4 h-4 animate-spin text-yellow-500" />}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {connectionStatus === "connecting" ? (
              <div className="flex flex-col items-center justify-center h-full space-y-4">
                ...
                <p className="text-lg font-medium text-gray-600">Finding you a chat buddy...</p>
